...
class RatingDynamicListView(View):
    ...
    def get(self, request, *args, **kwargs):
        return render(request, self.template_name, {"items": [], "title": "Динамика",})
...
class RatingUMOListView(View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        result_list = sorted(
            result_list_dont_sort, key=lambda i: i["rating"], reverse=True
        )
...
class RatingListView(View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        values = {
            ...
            "spirometry_yellow__max": D(
                Primary.objects.aggregate(Max("spirometry_yellow"))[
                    "spirometry_yellow__max"
                ]
            ),
            "speed__max": D(Primary.objects.aggregate(Max("speed"))["speed__max"]),
            "stamina__max": D(
                Primary.objects.aggregate(Max("stamina"))["stamina__max"]
            ),
        ...
        for i in items:
            rating = (
                ...
                + D(
                    (is_none_type(i["spirometry_yellow"]))
                    / values["spirometry_yellow__max"]
                )
                * D(1)
                + D((is_none_type(i["speed"])) / values["speed__max"]) * D(1)
                + D((is_none_type(i["stamina"])) / values["stamina__max"]) * D(1.2)
            )
        ...
        return render(
            request,
            self.template_name,
            {"items": result_list, "title": "Рейтинг первичных обследований",},
        )
...
class SportsmanListView(ListView):
    ...
    def get_queryset(self):
        ...
        if type_pk:
            new_queryset = Sportsman.objects.filter(sport_type_id=type_pk,)
            return new_queryset
        else:
