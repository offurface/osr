...
function getDirectories(path) {
  return fs.readdirSync(path).filter(function(file) {
    return (
      fs.statSync(`${path}/${file}`).isDirectory() && file !== "__pycache__"
    );
  });
}
...
gulp.task("templates", function() {
  return gulp.src(paths.templates).pipe(browserSync.reload({ stream: true }));
});
...
gulp.task("runServer", function(cb) {
  ...
  var cmd = `. ./${paths.env}/bin/activate && PYTHONUNBUFFERED=1 && `;
  ...
  var proc = exec(
    cmd + `python ./${pjson.name}/manage.py runserver ${serverUrl}`
  );
...
gulp.task("browserSync", function() {
  browserSync.init([paths.sass, paths.js, paths.templates], {
    proxy: serverUrl,
    port: 8080,
    reloadDelay: 300,
    reloadDebounce: 500
